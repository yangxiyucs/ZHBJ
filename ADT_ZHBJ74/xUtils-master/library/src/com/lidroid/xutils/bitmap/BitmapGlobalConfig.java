...
public class BitmapGlobalConfig {
	...
	private final static PriorityExecutor BITMAP_LOAD_EXECUTOR = new PriorityExecutor(
			DEFAULT_POOL_SIZE);
	private final static PriorityExecutor DISK_CACHE_EXECUTOR = new PriorityExecutor(
			2);
	...
	private final static HashMap<String, BitmapGlobalConfig> configMap = new HashMap<String, BitmapGlobalConfig>(
			1);
	...
	public synchronized static BitmapGlobalConfig getInstance(Context context,
		...
		if (configMap.containsKey(diskCachePath)) {
			...
			BitmapGlobalConfig config = new BitmapGlobalConfig(context,
					diskCachePath);
	...
	private void initBitmapCache() {
		...
		new BitmapCacheManagementTask()
				.execute(BitmapCacheManagementTask.MESSAGE_INIT_DISK_CACHE);
	}
	...
	public void setMemCacheSizePercent(float percent) {
		if (percent < 0.05f || percent > 0.8f) {
			throw new IllegalArgumentException(
					"percent must be between 0.05 and 0.8 (inclusive)");
		}
	...
	private int getMemoryClass() {
		return ((ActivityManager) mContext
	...
	private class BitmapCacheManagementTask extends
			PriorityAsyncTask<Object, Void, Object[]> {
		...
		protected void onPostExecute(Object[] params) {
			if (bitmapCacheListener == null || params == null
					|| params.length == 0)
				return;
			try {
				switch ((Integer) params[0]) {
				...
				case MESSAGE_CLEAR_BY_KEY:
					if (params.length != 2)
						return;
					bitmapCacheListener.onClearCacheFinished(String
							.valueOf(params[1]));
					break;
				case MESSAGE_CLEAR_MEMORY_BY_KEY:
					if (params.length != 2)
						return;
					bitmapCacheListener.onClearMemoryCacheFinished(String
							.valueOf(params[1]));
					break;
				case MESSAGE_CLEAR_DISK_BY_KEY:
					if (params.length != 2)
						return;
					bitmapCacheListener.onClearDiskCacheFinished(String
							.valueOf(params[1]));
					break;
				default:
					break;
	...
	public void clearCache() {
		new BitmapCacheManagementTask()
				.execute(BitmapCacheManagementTask.MESSAGE_CLEAR);
	}

	public void clearMemoryCache() {
		new BitmapCacheManagementTask()
				.execute(BitmapCacheManagementTask.MESSAGE_CLEAR_MEMORY);
	}

	public void clearDiskCache() {
		new BitmapCacheManagementTask()
				.execute(BitmapCacheManagementTask.MESSAGE_CLEAR_DISK);
	}

	public void clearCache(String uri) {
		new BitmapCacheManagementTask().execute(
				BitmapCacheManagementTask.MESSAGE_CLEAR_BY_KEY, uri);
	}
	...
	public void closeCache() {
		new BitmapCacheManagementTask()
				.execute(BitmapCacheManagementTask.MESSAGE_CLOSE);
	}
}